...
export const Users = (props: IUsersProps) => {
  ...
  const { usersList, match, loading } = props;
  return (
    <div>
      <h2 id="users-heading">
        <Translate contentKey="takeAHikeApp.users.home.title">Users</Translate>
        <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
          <FontAwesomeIcon icon="plus" />
          &nbsp;
          <Translate contentKey="takeAHikeApp.users.home.createLabel">Create new Users</Translate>
        </Link>
      </h2>
        ...
        {usersList && usersList.length > 0 ? (
          <Table responsive>
            ...
            <tbody>
              {usersList.map((users, i) => (
                <tr key={`entity-${i}`}>
                  <td>
                    <Button tag={Link} to={`${match.url}/${users.id}`} color="link" size="sm">
                      {users.id}
                    </Button>
                  </td>
                  ...
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`${match.url}/${users.id}`} color="info" size="sm">
                        <FontAwesomeIcon icon="eye" />{' '}
                        <span className="d-none d-md-inline">
                          <Translate contentKey="entity.action.view">View</Translate>
                        </span>
                      </Button>
                      <Button tag={Link} to={`${match.url}/${users.id}/edit`} color="primary" size="sm">
                        <FontAwesomeIcon icon="pencil-alt" />{' '}
                        <span className="d-none d-md-inline">
                          <Translate contentKey="entity.action.edit">Edit</Translate>
                        </span>
                      </Button>
                      <Button tag={Link} to={`${match.url}/${users.id}/delete`} color="danger" size="sm">
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="takeAHikeApp.users.home.notFound">No Users found</Translate>
            </div>
          )
        )}
      </div>
